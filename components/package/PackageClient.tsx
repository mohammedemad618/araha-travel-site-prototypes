...
type DepartureOption = { date: string; label: string; status: DepartureStatus };
...
/** Slide-in panel for the booking request, so the sticky card never outgrows the viewport. */
function Drawer({
  open,
  onClose,
  title,
...
export function EnquiryCard({
  ...
  return (
    <aside
      id="enquire"
      data-fab-avoid
      className="scroll-mt-28 rounded-[2px] bg-ink px-7 py-8 text-ivory lg:sticky lg:top-[96px]"
    >
      <div className="eyebrow mb-4 text-gold">{t('enquireEyebrow')}</div>
      <h2 className="m-0 mb-5 font-display text-[24px] leading-[1.3] font-medium">{t('enquireTitle')}</h2>
      <div className="mb-5 border-y border-ivory/14 py-5">
        <div className="mb-1.5 text-[13px] text-fog">{t('perPerson')}</div>
        <div className="flex flex-wrap items-baseline gap-x-3">
          <Price value={price} className="text-[38px] leading-none" unitClassName="text-base text-gold" />
          {usd && (
      ...
      <div className="mb-2.5 text-[13px] text-fog" id="dep-label">
        {t('departures')}
      </div>
      {options.length > 0 ? (
        <div className="mb-5 flex flex-col gap-1.5" role="radiogroup" aria-labelledby="dep-label">
          {options.map((d, i) => {
            const checked = sel === i;
            ...
            return (
              ...
              >
                <span>{d.label}</span>
                <span
                  className={`flex items-center gap-1.5 text-[12px] no-underline ${checked ? 'text-muted' : 'text-fog'}`}
                >
                  <span className={`h-1.5 w-1.5 rounded-full ${STATUS_STYLE[d.status]}`} aria-hidden="true" />
                  {t(`status.${d.status}`)}
                </span>
              </button>
            );
          ...
          {picked?.status === 'soldout' && (
            <p className="m-0 mt-1 text-[12.5px] text-sand">{t('soldOutNote')}</p>
          )}
        </div>
      ) : (
      ...
      <div className="mb-4 border-t border-ivory/14 pt-3">
      ...
      <div className="flex flex-col gap-2.5">
        <WhatsAppLink
          ...
          className="flex items-center justify-between rounded-[1px] bg-gold px-5 py-4 text-[15.5px] font-medium text-ink transition-colors hover:bg-sand hover:text-ink"
        >
        ...
        <button
          ...
          className="flex items-center justify-between rounded-[1px] border border-ivory/40 px-5 py-4 text-[15px] text-ivory transition-colors hover:border-ivory"
        >
      ...
      <ul className="m-0 mt-5 flex list-none flex-col gap-1.5 border-t border-ivory/10 p-0 pt-4 text-[12.5px] text-fog">
      ...
      <Drawer open={drawer} onClose={() => setDrawer(false)} title={t('bookingTitle')}>
...
export function MobileBookingBar({ price }: { price: number }) {
  ...
  return (
    <div className="fixed inset-x-0 bottom-0 z-70 flex items-center gap-2.5 border-t border-ivory/10 bg-ink/97 px-3.5 pt-2.5 pb-[calc(10px+env(safe-area-inset-bottom))] text-ivory backdrop-blur-[14px] md:inset-x-auto md:start-1/2 md:bottom-4 md:w-[min(560px,calc(100%-32px))] md:-translate-x-1/2 md:rounded-[4px] md:border md:rtl:translate-x-1/2 lg:hidden">
